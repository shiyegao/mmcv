...
class OnlineMeanVarBatchNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        self.momentum_v = 0.5
        self.momentum_m = 0.5
        scale = self.weight * ((1 - self.momentum_v) * self.running_var + current_var * self.momentum_v + 1e-5).rsqrt()
        bias = self.bias - ((1 - self.momentum_m) * self.running_mean + current_mean * self.momentum_m) * scale
...
class SelfMeanVarBatchNorm2d(nn.Module):
    __constants__ = ['num_features']
    def __init__(self, num_features, eps):
        # super().__init__(num_features)
        super().__init__()
    ...
    def forward(self, x):
        bs = x.size(0)
        current_mean = x.mean([2, 3]).reshape(bs, -1)  # bs x D
        current_var = x.var([2, 3], unbiased=False).reshape(bs, -1)  # bs x D
        if self.weight.dim() == 1: # batch
            scale = self.weight.repeat(bs, 1) * (current_var + 1e-5).rsqrt() # bs x D
            bias = self.bias.repeat(bs, 1) - current_mean * scale  # bs x D
            scale = scale.reshape(bs, -1, 1, 1)
            bias = bias.reshape(bs, -1, 1, 1)
        else: # sample
            scale = (current_var + 1e-5).rsqrt().mul(self.weight.reshape(bs, -1))  # bs x D
            bias = self.bias.reshape(bs, -1) - current_mean * scale  # bs x D
            scale = scale.reshape(bs, -1, 1, 1)
            bias = bias.reshape(bs, -1, 1, 1)
        return x * scale + bias
...
NORM_LAYERS.register_module('smvBN', module=SelfMeanVarBatchNorm2d)
