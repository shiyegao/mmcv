...
# TODO: make sample_norm compatible with paramwise.
# current method is either paramwise, sample_norm, or normal version.
# We may as well add sample_norm in the function: add_params().2 
@OPTIMIZER_BUILDERS.register_module()
class SampleNormOptimizerConstructor:
    ...
    def __init__(self, optimizer_cfg, paramwise_cfg=None):
        ...
        self.sample_norm_batch_size = optimizer_cfg.pop('sample_norm_batch_size', -1)
    ...
    def __call__(self, model):
        ...
        if not self.paramwise_cfg:
            if self.sample_norm_batch_size > 0:
                optimizer_cfg['params'] = self.collect_params(model, self.sample_norm_batch_size)
            else:
    ...
    def collect_params(self, model, bs):
        params, pnames, bn_names = [], [], set()
        ...
        # norm func already takes care of the reset part
        # for np, p in model.named_parameters():
        #     if np in bn_names:
        #         if len(p.shape) == 2:
        #             p = torch.nn.Parameter(torch.mean(p, 0))
        # model.load_state_dict(model_state, strict=True)
        
        for np, p in model.named_parameters():
            if np in bn_names:
                p = torch.nn.Parameter(p.unsqueeze(0).repeat(bs, 1))
